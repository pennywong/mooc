...
void* mm_malloc (size_t size) {
  ...
  if((blockSize - reqSize) > MIN_BLOCK_SIZE){
      ...
      *((size_t*)POINTER_ADD(ptrFreeBlock, (reqSize-WORD_SIZE)))=reqSize;
      *((size_t*)POINTER_ADD(ptrFreeBlock, (blockSize-WORD_SIZE)))=blockSize-reqSize;
        
      // Inserts the new block into the free list
      insertFreeBlock(newBlock);
  } else {
      // Updates the following blocks preceding used and the boundary tag
      *((size_t*)POINTER_ADD(ptrFreeBlock, (blockSize-WORD_SIZE)))=blockSize;
