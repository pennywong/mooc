...
int main(int argc, char* argv[]) {
  ...
  // testing.
  // Answer: Filling the array with 11 or 100 or 1000 values
  fillArray(array, 10);
  ...
  // Answer: Because the array is just a pointer to an integer
  fillArray(&value, 1);
  ...
  // C++ programming.
  // Answer: leak memory, 20 bytes allocated but not freed
  free(heap_array);
  ...
  FourInts* four_ints_d = (FourInts*) malloc(sizeof(FourInts));
  fillArray((int*)four_ints_d, 4);
  
  assert(four_ints_d->a == 2);
  assert(four_ints_d->b == 5);
  assert(four_ints_d->c == 8);
  assert(four_ints_d->d == 11);
